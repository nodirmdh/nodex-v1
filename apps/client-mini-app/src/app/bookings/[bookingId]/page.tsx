...
export default function BookingDetailPage() {
  ...
  return (
    <ClientShell active="trips">
      <ClientHeader backHref="/bookings" level="secondary" title="ENVO Protection" subtitle="Nukus to Urgench" />
      ...
      <Card className="mt-4 space-y-3" compact label="Trip reliability state">
        ...
        <div className="grid grid-cols-2 gap-2">{(Object.keys(delayCopy) as DelayState[]).map((key) => <button key={key} className={`min-h-10 rounded-[16px] border-0 px-2 text-xs font-black ${delayState === key ? "bg-[rgb(var(--primary))] text-[rgb(var(--primary-foreground))]" : "bg-[rgb(var(--canvas))] text-[rgb(var(--text-muted))]"}`} type="button" onClick={() => setDelayState(key)}>{delayCopy[key].label}</button>)}</div>
        <div className="grid grid-cols-3 gap-2"><Link className="inline-flex min-h-11 items-center justify-center rounded-full bg-[rgb(var(--canvas))] px-3 text-center text-xs font-black text-[rgb(var(--primary))] no-underline" href="/messages/driver-azizbek">Написать</Link><Link className="inline-flex min-h-11 items-center justify-center rounded-full bg-[rgb(var(--canvas))] px-3 text-center text-xs font-black text-[rgb(var(--primary))] no-underline" href="/support?tripId=phase6-booking-confirmed">Поддержка</Link><button className="min-h-11 rounded-full border-0 bg-[rgb(var(--canvas))] px-3 text-xs font-black text-[rgb(var(--primary))]" type="button" onClick={() => setDelayState("ON_TIME")}>Подождать</button></div>
      ...
      <Card className="mt-3 space-y-3" compact label="ENVO Protection">
        ...
        {protection === "idle" ? <button className="min-h-11 w-full rounded-full border-0 bg-[rgb(var(--foreground))] px-4 text-sm font-black text-[rgb(var(--primary-foreground))]" type="button" onClick={simulateLateCancellation}>Demo: водитель отменил</button> : null}
