...
export default function AdminParcelsPage() {
  ...
  return (
    <main className="admin-main">
      <AdminPageHeader title="Посылки" subtitle="Отправитель, получатель, маршрут, водитель, статус и контекст поддержки." actions={<><button className="rounded-[10px] border border-[rgb(var(--border))] bg-[rgb(var(--surface))] px-3 py-2 text-sm font-black" type="button" onClick={() => setNotice("Показаны активные посылки.")}>Активные</button><button className="rounded-[10px] bg-[rgb(var(--primary))] px-3 py-2 text-sm font-black text-[rgb(var(--primary-foreground))]" type="button" onClick={() => setSelectedId("PCL-1043")}>Проверить выдачу</button></>} />
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
        ...
        <AdminPanel className="p-4" label="Детали посылки"><div className="flex items-start justify-between gap-3"><div><h2 className="m-0 text-xl font-black">{selected.id}</h2><p className="m-0 text-sm font-semibold text-[rgb(var(--text-muted))]">{selected.origin} → {selected.destination}</p></div><AdminStatusBadge tone={tone(selected.status)}>{selected.status}</AdminStatusBadge></div><div className="mt-4 grid gap-2 text-sm"><Info label="Участники" value={`${selected.sender} → ${selected.receiver}`} /><Info label="Водитель" value={selected.driver} /><Info label="Размер и описание" value={`${selected.size} · ${selected.note}`} /><Info label="Стоимость" value={formatUzs(selected.priceMinor)} /><Info label="Контекст поддержки" value={selected.support} /></div><div className="mt-4 grid grid-cols-2 gap-2"><button className="rounded-[10px] border border-[rgb(var(--border))] px-3 py-2 text-sm font-black" type="button" onClick={() => setNotice("Открыта связанная поездка.")}>Открыть рейс</button><button className="rounded-[10px] border border-[rgb(var(--border))] px-3 py-2 text-sm font-black" type="button" onClick={() => setNotice("Контекст поддержки открыт.")}>Поддержка</button><button className="rounded-[10px] border border-[rgb(var(--warning))] px-3 py-2 text-sm font-black text-[rgb(var(--warning))]" type="button" onClick={() => setNotice("Создана заметка проверки выдачи.")}>Проверка</button><button className="rounded-[10px] border border-[rgb(var(--destructive))] px-3 py-2 text-sm font-black text-[rgb(var(--destructive))]" type="button" onClick={() => setNotice("Открыта проверка отмены.")}>Отмена</button></div>{notice ? <div className="mt-3 rounded-[12px] bg-[rgb(var(--info-soft))] p-3 text-sm font-black text-[rgb(var(--info))]">{notice}</div> : null}<div className="mt-4 grid gap-2 text-sm"><h3 className="m-0 text-sm font-black">История статуса</h3>{["Создана", "Водитель назначен", "Принята водителем", "Забрана", "В пути", "Доставлена"].map((label, index) => <div key={label} className="grid grid-cols-[24px_1fr] gap-2"><span className={`mt-1 h-3 w-3 rounded-full ${index <= 4 ? "bg-[rgb(var(--primary))]" : "bg-[rgb(var(--border-strong))]"}`} /><span>{label}</span></div>)}</div></AdminPanel>
      </div>
    </main>
  );
}
