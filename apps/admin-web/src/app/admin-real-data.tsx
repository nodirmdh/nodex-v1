...
function useAdminAccessToken() {
  ...
  const login = useCallback(async () => {
    ...
    if (!response.ok) throw new Error("Не удалось выполнить вход администратора");
...
export function TripDetailRealData({ tripId }: { tripId: string }) {
  ...
  if (!accessToken) return <DemoTripDetail tripId={tripId} note="Показана доступная поездка." />;
  if (tripState.status === "loading") return <EmptyPanel title="Детали поездки">Загружаем поездку...</EmptyPanel>;
  if (tripState.status === "error") return <DemoTripDetail tripId={tripId} note={`Данные временно недоступны: ${tripState.error}. Показаны сохранённые данные.`} />;
  ...
  return <div className="admin-detail-layout"><AdminPanel className="p-4"><DetailGrid items={[["Водитель", trip.driverProfile?.id ? <LinkedValue key="d" href={`/drivers/${trip.driverProfile.id}`}>{userName(trip.driverProfile.user)}</LinkedValue> : userName(trip.driverProfile?.user)], ["Автомобиль", vehicleName(trip)], ["Статус", <Status key="s" value={trip.status} />], ["Выезд", formatDate(trip.departureAtUtc)], ["Места", `${trip.availableSeatCount}/${trip.passengerSeatCapacity}`], ["Бронирования", String(bookings.length)], ["Весь автомобиль", trip.wholeCarPriceMinor ? `${trip.wholeCarPriceMinor}` : "-"], ["Start PIN", bookings.some((booking) => Array.isArray(booking.startPins) && booking.startPins.length) ? <Status key="p" value="Есть" /> : "Нет данных"], ["ETA", formatDate(trip.arrivalEstimateAtUtc)], ["GPS", Array.isArray(operations?.locations) ? `${operations.locations.length} точек` : "Используйте историю поездки"], ["Надёжность", trip.driverProfile?.reliabilityScore == null ? "-" : String(trip.driverProfile.reliabilityScore)]]} /></AdminPanel><Tabs tabs={[{ label: "Пассажиры", content: bookings.length ? bookings.map((booking) => <Row key={String(booking.id)} href={`/bookings/${booking.id}`} title={userName(booking.client)} meta={String(booking.status ?? "BOOKING")} />) : <span className="text-sm text-[rgb(var(--text-muted))]">Пассажиров пока нет.</span> }, { label: "Бронирования", content: bookings.length ? bookings.map((booking) => <Row key={String(booking.id)} href={`/bookings/${booking.id}`} title={String(booking.id)} meta={String(booking.status ?? "BOOKING")} />) : <span className="text-sm text-[rgb(var(--text-muted))]">Бронирований пока нет.</span> }, { label: "История", content: <EventList items={timeline} /> }, { label: "История GPS", content: <JsonBlock value={operations?.locations ?? operations?.locationHistory ?? "История геолокации пока недоступна."} /> }, { label: "Фрод-флаги", content: <EventList items={moderation} /> }, { label: "Награды", content: <JsonBlock value={operations?.rewards ?? "Связанных наград пока нет."} /> }, { label: "Поддержка", content: <Link className="font-black text-[rgb(var(--primary))] no-underline hover:underline" href={`/support?tripId=${trip.id}`}>Открыть обращения по поездке</Link> }, { label: "Подбор", content: <JsonBlock value={operations?.matching ?? operations?.waitlist ?? "Связанных данных подбора пока нет."} /> }]} /></div>;
}

function EventList({ items }: { items: Array<Record<string, unknown>> }) {
  if (!items.length) return <span className="text-sm text-[rgb(var(--text-muted))]">Событий пока нет.</span>;
