...
const passengers = [
  { name: "A. Karimov", initials: "AK", seat: "Переднее место", status: "На борту" },
...
const parcels = [
  {
    ...
    detail: "Nukus, вокзал → Urgench, автостанция",
    status: "Принято",
  },
];
...
export default function TripOperationDemo() {
  ...
  const [cancelled, setОтменена] = useState(false);
  ...
  const isЗавершена = state === "completed";
  const boarded = 2;

  return (
    <DriverShell active="trips">
      <DriverHeader
        title={cancelled ? "Отменена" : isActive ? "В пути" : isЗавершена ? "Завершена" : "Поездка"}
        subtitle="Nukus → Urgench · Chevrolet Cobalt"
        status={
          <DriverPill tone={isЗавершена ? "success" : isActive ? "warning" : "info"}>
            {cancelled ? "Отменена" : isЗавершена ? "Завершена" : isActive ? `В пути · ${reportedDelay}` : "Посадка"}
          </DriverPill>
        }
      />
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="m-0 text-2xl font-semibold">Nukus → Urgench</h1>
            <p
              className={[
        ...
        >
          ...
          <Metric label="Ожидает" value={isЗавершена ? "0" : "1"} active={isActive} />
        </div>
        {cancelled ? (
          <p className="m-0 text-sm font-semibold text-[rgb(var(--destructive))]">Демо-отмена сохранена. Событие надёжности остаётся видимым для проверки.</p>
        ) : isActive ? (
          <p className="m-0 text-sm font-semibold opacity-80">
            Поездка началась. Пассажиры и посылка остаются доступны до прибытия.
          </p>
        ) : isЗавершена ? (
          <p className="m-0 text-sm font-semibold text-[rgb(var(--text-muted))]">
            Поездка завершена: 2 пассажира на борту, история неявки сохранена.
          </p>
        ) : (
          <p className="m-0 text-sm font-semibold text-[rgb(var(--text-muted))]">
      ...
      {!isЗавершена && (
        <DriverCard className="mt-3 space-y-3" label="Trip next operation">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="m-0 text-lg font-semibold">
                {isActive ? "Завершите после прибытия" : "Начать поездку?"}
              </h2>
              <p className="m-0 text-sm font-semibold text-[rgb(var(--text-muted))]">
                {isActive
                  ? "Проверьте пассажиров и посылку перед завершением."
                  : "2 из 3 пассажиров на борту · 1 пассажир не решён."}
              </p>
            </div>
            <DriverPill tone={isActive ? "warning" : "info"}>
              {isActive ? "Прибытие" : "Готово"}
            </DriverPill>
          </div>
          {!isActive ? (
            <div className="grid gap-2 rounded-[20px] bg-[rgb(var(--canvas))] p-3">
              <label
                className="text-xs font-semibold uppercase tracking-[0.12em] text-[rgb(var(--primary))]"
              ...
              <input
                id="trip-start-pin"
                className="min-h-12 rounded-full border border-[rgb(var(--border))] bg-[rgb(var(--surface))] px-4 text-center text-xl font-semibold tracking-[0.25em] outline-none"
                inputMode="numeric"
                maxLength={4}
          ...
          <button
            className="min-h-12 w-full rounded-full border-0 bg-[rgb(var(--primary))] px-4 text-sm font-semibold text-[rgb(var(--primary-foreground))] disabled:opacity-50"
            disabled={!isActive && startPin.length !== 4}
            onClick={() => (isActive ? setFinishSheet(true) : setState("active"))}
            type="button"
          >
            {isActive ? "Завершить поездку" : "Проверить PIN и начать"}
          </button>
          {finishSheet && (
            <FinishSheet
              onFinish={() => {
      ...
      {!isЗавершена && !cancelled ? (
        <DriverCard className="mt-3 space-y-3" label="Действия надёжности">
          <div className="flex items-start justify-between gap-3"><div><h2 className="m-0 text-lg font-semibold">Действия надёжности</h2><p className="m-0 text-sm font-semibold text-[rgb(var(--text-muted))]">Сообщите о задержке или отмене с видимым влиянием на надёжность.</p></div><DriverPill tone={reportedDelay === "По расписанию" ? "success" : "warning"}>{reportedDelay}</DriverPill></div>
          <div className="grid grid-cols-2 gap-2"><button className="min-h-11 rounded-full border-0 bg-[rgb(var(--canvas))] px-3 text-sm font-semibold text-[rgb(var(--primary))]" type="button" onClick={() => setDelayOpen(true)}>Сообщить о задержке</button><button className="min-h-11 rounded-full border-0 bg-[rgb(var(--destructive-soft))] px-3 text-sm font-semibold text-[rgb(var(--destructive))]" type="button" onClick={() => setCancelOpen(true)}>Отменить поездку</button></div>
        </DriverCard>
      ) : null}
      ...
      <DriverCard className="mt-3 space-y-3" label="Занятость мест">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="m-0 text-lg font-semibold">Занятость мест</h2>
            <p className="m-0 text-sm font-semibold text-[rgb(var(--text-muted))]">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ["Переднее", "На борту"],
            ["Заднее левое", isЗавершена ? "На борту" : "Подтверждён"],
            ...
            <div
              key={seat}
              className={[
                "rounded-[16px] p-2 text-center text-[10px] font-semibold",
                status === "На борту"
      ...
      <DriverCard className="mt-3 space-y-3" label="Пассажиры">
        <div className="flex items-center justify-between gap-3">
          <h2 className="m-0 text-lg font-semibold">Пассажиры</h2>
          <Link
            className="text-sm font-semibold text-[rgb(var(--primary))] no-underline"
        ...
        {passengers.map((passenger) => (
          ...
          >
            <span className="grid h-10 w-10 place-items-center rounded-full bg-[rgb(var(--surface-tint))] text-xs font-semibold text-[rgb(var(--primary))]">
              {passenger.initials}
            </span>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-semibold">{passenger.name}</div>
            ...
            <PassengerStatusPill
              status={
                isЗавершена && passenger.status === "Подтверждён" ? "На борту" : passenger.status
              }
            />
          </div>
        ))}
      ...
      <DriverCard className="mt-3 space-y-3" label="Посылки">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="m-0 text-lg font-semibold">Есть посылка по маршруту</h2>
        ...
        {parcels.map((parcel) => (
          <div key={parcel.title} className="rounded-[18px] bg-[rgb(var(--canvas))] p-3">
            <div className="text-sm font-semibold">{parcel.title}</div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {["Забрал посылку", "В пути", "Передал получателю"].map((action) => (
                <button key={action} className={`min-h-10 rounded-full border-0 px-2 text-[11px] font-semibold ${parcelState === action ? "bg-[rgb(var(--primary))] text-[rgb(var(--primary-foreground))]" : "bg-[rgb(var(--surface))] text-[rgb(var(--primary))]"}`} type="button" onClick={() => setParcelState(action)}>{action}</button>
              ))}
            </div>
          </div>
        ))}
      ...
      {cancelOpen ? <CancelSheet reason={cancelReason} onReason={setCancelReason} onClose={() => setCancelOpen(false)} onConfirm={() => { setОтменена(true); setCancelOpen(false); }} /> : null}
    </DriverShell>
  );
}
...
function DelaySheet({ onClose, onSave }: { onClose: () => void; onSave: (value: string) => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-end bg-[rgb(var(--foreground)/0.28)] p-3" role="dialog" aria-modal="true"><section className="w-full max-w-[430px] rounded-[26px] bg-[rgb(var(--surface))] p-4 shadow-[var(--shadow-floating)]"><div className="flex items-center justify-between gap-3"><h2 className="m-0 text-lg font-semibold">Сообщить о задержке</h2><button className="h-9 w-9 rounded-full border-0 bg-[rgb(var(--canvas))] text-lg font-semibold" type="button" onClick={onClose}>×</button></div><div className="mt-3 grid grid-cols-3 gap-2">{["5 минут", "10 минут", "15 минут", "30+ минут", "своё время"].map((item) => <button key={item} className={`min-h-10 rounded-[16px] border-0 px-2 text-xs font-semibold ${minutes === item ? "bg-[rgb(var(--primary))] text-[rgb(var(--primary-foreground))]" : "bg-[rgb(var(--canvas))] text-[rgb(var(--text-muted))]"}`} type="button" onClick={() => setMinutes(item)}>{item}</button>)}</div><div className="mt-3 grid grid-cols-2 gap-2">{["пробки", "пассажир задерживается", "техническая причина", "другое"].map((item) => <button key={item} className={`min-h-10 rounded-[16px] border-0 px-2 text-xs font-semibold ${reason === item ? "bg-[rgb(var(--surface-tint))] text-[rgb(var(--primary))]" : "bg-[rgb(var(--canvas))] text-[rgb(var(--text-muted))]"}`} type="button" onClick={() => setReason(item)}>{item}</button>)}</div><button className="mt-3 min-h-11 w-full rounded-full border-0 bg-[rgb(var(--primary))] px-4 text-sm font-semibold text-[rgb(var(--primary-foreground))]" type="button" onClick={() => onSave(`${minutes} · ${reason}`)}>Сохранить ETA</button></section></div>;
}
...
function CancelSheet({ reason, onReason, onClose, onConfirm }: { reason: string; onReason: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  return <div className="fixed inset-0 z-50 grid place-items-end bg-[rgb(var(--foreground)/0.28)] p-3" role="dialog" aria-modal="true"><section className="w-full max-w-[430px] rounded-[26px] bg-[rgb(var(--surface))] p-4 shadow-[var(--shadow-floating)]"><div className="flex items-center justify-between gap-3"><h2 className="m-0 text-lg font-semibold">Отмена поездки</h2><button className="h-9 w-9 rounded-full border-0 bg-[rgb(var(--canvas))] text-lg font-semibold" type="button" onClick={onClose}>×</button></div><p className="m-0 mt-2 rounded-[18px] bg-[rgb(var(--destructive-soft))] p-3 text-sm font-semibold text-[rgb(var(--destructive))]">Отмена за 25 минут до выезда снизит показатель надёжности.</p><div className="mt-3 grid gap-2">{["Техническая причина", "Пробки", "Автомобиль недоступен", "Проблема с пассажиром", "Другое"].map((item) => <button key={item} className={`min-h-10 rounded-[16px] border-0 px-3 text-left text-sm font-semibold ${reason === item ? "bg-[rgb(var(--primary))] text-[rgb(var(--primary-foreground))]" : "bg-[rgb(var(--canvas))] text-[rgb(var(--foreground))]"}`} type="button" onClick={() => onReason(item)}>{item}</button>)}</div><button className="mt-3 min-h-11 w-full rounded-full border-0 bg-[rgb(var(--destructive-soft))] px-4 text-sm font-semibold text-[rgb(var(--destructive))]" type="button" onClick={onConfirm}>Подтвердить отмену</button></section></div>;
}
...
function FinishSheet({ onFinish }: { onFinish: () => void }) {
  return (
    <div className="rounded-[22px] bg-[rgb(var(--foreground))] p-4 text-[rgb(var(--primary-foreground))] shadow-[var(--shadow-floating)]">
      <h3 className="m-0 text-lg font-semibold">Завершить поездку?</h3>
      <p className="m-0 mt-1 text-sm font-semibold opacity-80">
        Nukus → Urgench · 3 пассажира · 1 посылка
      </p>
      <button
